...
class SearchMovieCubit extends Cubit<SearchMovieState> {
  ...
  SearchMovieCubit({@required this.loadingCubit, @required this.searchMovies})
  ...
  void searchTermChanged(String searchTerm) async {
    ...
    if (searchTerm.length > 2) {
      ...
      final Either<AppError, List<MovieEntity>> response = await searchMovies(
        MovieSearchParams(searchTerms: searchTerm),
      );
      emit(response.fold((l) => SearchMovieError(l.appErrorType),
